...
def main():
    ...
    try:
        ...
        overwrite_description(file_path, description)

        # Tag aus dem Dateinamen extrahieren und überschreiben
        day = get_date_from_filename(file_path)
        overwrite_day(file_path, day)
        ...
        season = get_tvsn(metadata)
        target_dir = move_file(file_path, album, season)  # Speichere das Zielverzeichnis
